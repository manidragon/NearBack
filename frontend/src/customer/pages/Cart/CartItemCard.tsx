...
const getSellerName = (item: CartItem): string => {
  ...
  if (item.sellerName && item.sellerName.trim() !== '') {
    console.log('✅ [getSellerName] Using direct sellerName:', item.sellerName);
  ...
  if (item.sellerId && typeof item.sellerId === 'object') {
    ...
    console.log('✅ [getSellerName] Using populated sellerId:', {
      businessName,
      sellerName,
      finalName: businessName || sellerName || 'Seller'
    });
  ...
  if (typeof item.sellerId === 'string' && item.product?.variants && item.variantId) {
    ...
    if (variant?.offers) {
      ...
      if (matchingOffer && typeof matchingOffer.seller === 'object') {
        ...
        console.log('✅ [getSellerName] Using variant offer match:', name);
        return name || 'Seller';
      }
    }
  }
  ...
  if (item.product?.seller && typeof item.product.seller === 'object') {
    ...
    const name = seller.businessDetails?.businessName || seller.sellerName;
    console.log('⚠️ [getSellerName] FALLBACK to product.seller:', name);
    return name || 'Seller';
  }
  
  console.log('❌ [getSellerName] No seller found, returning "Seller"');
  return 'Seller';
};
